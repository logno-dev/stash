      ...
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ children }) => <h3 className="text-base font-medium text-white mb-1">{children}</h3>,
            p: ({ children }) => <p className="text-slate-300 mb-2 last:mb-0 break-words">{children}</p>,
          strong: ({ children }) => <strong className="font-semibold text-white">{children}</strong>,
          em: ({ children }) => <em className="italic text-zinc-200">{children}</em>,
            code: ({ children }) => (
              <code className="bg-slate-800 text-slate-200 px-1 py-0.5 rounded text-sm font-mono">
                {children}
              </code>
            ),
          pre: ({ children }) => (
          ...
          blockquote: ({ children }) => (
              <blockquote className="border-l-4 border-slate-500 pl-4 my-2 text-slate-300 italic">
                {children}
              </blockquote>
            ),
          ...
          li: ({ children }) => <li className="mb-1">{children}</li>,
            a: ({ href, children }) => (
              <a 
                ...
                className="text-slate-300 hover:text-slate-100 underline break-words"
              >
                {children}
              </a>
            ),
